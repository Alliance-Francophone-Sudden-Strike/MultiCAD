...
int main()
{
    ...
    constexpr int pitch = 8, rows = 6, split = 4;
    std::array<uint16_t, pitch * (rows + 1)> regionMain{}, regionBack{};
    const auto fill = [&]
    {
        for (size_t i = 0; i < regionMain.size(); ++i)
        {
            regionMain[i] = static_cast<uint16_t>(100 + i);
            regionBack[i] = static_cast<uint16_t>(200 + i);
        }
    };
    fill();
    const auto cleanMain = regionMain;
    const auto cleanBack = regionBack;

    SurfaceView view{ regionMain.data(), regionBack.data(), pitch, rows,
                      (rows - split) * pitch, split };
    assert(view.offset(0) == (rows - split) * pitch);
    assert(view.offset(split - 1) == (rows - 1) * pitch);
    assert(view.offset(split) == 0);
    assert(view.offset(rows - 1) == (rows - 1 - split) * pitch);

    State region;
    region.setMode(Mode::On);
    region.addWheelDelta(120);

    const Rect band{ 2, 3, 3, 2 }; // straddles the seam at split
    region.beginRegionIsolation(view, band);
    for (auto& pixel : regionMain)
        pixel = 7;
    for (auto& pixel : regionBack)
        pixel = 9;
    region.finishRegionIsolation();

    for (int y = 0; y < rows; ++y)
        for (int x = 0; x < pitch; ++x)
        {
            const auto at = view.offset(y) + x;
            const bool inside = x >= band.x && x < band.x + band.width &&
                y >= band.y && y < band.y + band.height;
            assert(regionMain[at] == (inside ? cleanMain[at] : 7));
            assert(regionBack[at] == (inside ? cleanBack[at] : 9));
        }

    fill();
    State offRegion;
    offRegion.setMode(Mode::On);
    assert(!offRegion.presenting());
    offRegion.beginRegionIsolation(view, band);
    regionMain[view.offset(band.y) + band.x] = 42;
    offRegion.finishRegionIsolation();
    assert(regionMain[view.offset(band.y) + band.x] == 42);

    fill();
    region.beginRegionIsolation(view, { -4, -4, 2, 2 });
    regionMain[0] = 55;
    region.finishRegionIsolation();
    assert(regionMain[0] == 55);
